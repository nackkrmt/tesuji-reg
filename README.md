...
- ↩️ **ถอนตัว (Withdraw)** — เจ้าของใบสมัครถอนผู้เล่นออกเป็น **รายที่นั่ง** ได้เองจาก "ใบสมัครของฉัน" (ได้ทั้งใบที่ยืนยันแล้วและรอตรวจสลิป, ไม่มีกำหนดเส้นตาย) โดยกรอกบัญชีรับเงินคืน + เหตุผล — **การคืนเงินอยู่ในดุลยพินิจผู้จัด จัดการนอกระบบ**; ที่นั่งคืนเข้าโควตาให้คนอื่นสมัครต่อ, ชื่อหายจากรายชื่อสาธารณะ, แต่ **ยอดเงินของใบสมัครไม่เปลี่ยน** (ยอดรายได้ในแดชบอร์ดคงเดิม)
...
- 🧑‍⚖️ **จัดการกรรมการ** — แต่งตั้ง/ถอดถอนสิทธิ์กรรมการเป็นรายบัญชี (role `judge`; บัญชีเดียวถือได้ทั้ง `admin` + `judge`) ผ่าน `admin_set_judge` / `admin_list_judges`
...
| **แอดมิน** | |
| `/admin/*` | หลังบ้าน: `login`, `tournament`, `categories`, `registrations`, `registrations/[id]`, `withdrawals`, `live`, `database`, `institutes`, `codes`, `reset` |
...
│  ├─ admin/                  # หลังบ้าน (tournament, categories, registrations, withdrawals, live, database, institutes, codes, reset)
...
│  └─ PublicHeader.tsx, LanguageSwitcher.tsx, GlassDock.tsx, RulesPdfViewer.tsx
├─ lib/
│  ├─ data/                   # ★ DataLayer seam
...
│  └─ migrations/              # SQL changelog 23 ไฟล์ (โค้ดส่วนลด → hardening → แข่งสด → roles → เพดานรางวัล → กันซ้ำข้ามบัญชี → ถอนตัว/เปลี่ยนคน) — ⚠️ ไม่รวม base schema
...
- **RPC สำคัญ (จัดกลุ่ม):**
  ...
  | | `admin_list_withdrawals` / `admin_set_withdrawal_status` | แอดมินดูรายการถอนตัว + ตั้งสถานะคืนเงิน `pending`/`refunded`/`denied` |
  ...
  | ฐานฝีมือ + เพดานรางวัล | `search_go_player_database` | ค้นระดับฝีมือจากชื่อ (exact → normalized → fuzzy) |
  | | `replace_go_player_database_source` | นำเข้า/แทนที่ฐาน DAN/KYU/AWARD ทีละแหล่งทั้งชุด |
  ...
  | จัดการงาน/รีเซ็ต | `admin_clear_registrations` / `admin_clear_categories` / `admin_delete_tournament` | รีเซ็ตหลังจบงาน — ต้องส่งชื่อรายการยืนยัน |
