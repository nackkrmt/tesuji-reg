...
- 📋 ตรวจ-ยืนยัน/ปฏิเสธใบสมัคร + ดูสลิป — **ค้นหารายชื่อ** (แสดงเป็นรายคน + ยอดที่ต้องโอน), **แก้ไข/ลบผู้สมัครรายคนหรือทั้งใบ** (ปรับที่นั่ง/ยอดเงินให้อัตโนมัติ), หน้ารวมรายชื่อผู้เข้าแข่งขัน
...
| `/admin/*` | หลังบ้าน: `login`, `tournament`, `categories`, `registrations`, `registrations/[id]`, `database` |
...
│  ├─ admin/                  # หลังบ้าน (tournament, categories, registrations, database)
...
- **RPC สำคัญ:** `reserve_seats` (จองที่นั่ง all-or-nothing + กันโกงรุ่นทั้งระดับฝีมือ/อายุ), `release_expired_holds` (คืนที่นั่งหมดเวลา — pg_cron ทุกนาที + lazy), `search_go_player_database`, `replace_go_player_database_source`, `admin_update_seat` / `admin_delete_seat` / `admin_delete_batch` (แก้ไข/ลบผู้สมัคร) และ RPC ฝั่งแอดมินสำหรับจัดการรายการแข่ง/รุ่น/ใบสมัคร
