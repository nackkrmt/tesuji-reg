...
- 👨‍👩‍👧 **ผู้เล่นในความดูแล (managed players)** — โค้ช/ผู้ปกครองบันทึกรายชื่อลูกทีมไว้ใช้สมัครซ้ำได้ (กรอกชื่อไทย/วันเกิดก่อนได้ ชื่ออังกฤษ/เบอร์โทรไม่บังคับ เติมภายหลังได้)
...
- 💳 **PromptPay / Thai-QR + อัปสลิป** — สร้าง QR ตามยอด รองรับทั้งพร็อกซีส่วนตัว (เบอร์/บัตร ปชช.) และ **QR ร้านค้า (K SHOP merchant QR)** โดยฝังยอดเงินลงใน payload ของร้านโดยตรง แล้วอัปโหลดสลิปเพื่อรอตรวจ
...
- ⚙️ ตั้งค่ารายการแข่ง (ชื่อ, แบนเนอร์, วันเวลาเปิด-ปิดรับ, ผู้รับเงิน PromptPay/Thai-QR, กำหนดการ, กติกา)
...
- 🤖 **ตรวจสลิปอัตโนมัติ (SlipOK)** — กดยืนยันสลิปทีละใบ เทียบยอด/ผู้รับ/ตรวจสลิปซ้ำ ผ่าน Edge Function (คีย์อยู่ฝั่งเซิร์ฟเวอร์); ยังไม่ตั้งคีย์ → ทำงานแบบ **เดโม (ผลจำลอง)** ได้ทันที
...
| `NEXT_PUBLIC_DEFAULT_MERCHANT_QR` | (ออปชัน) Thai-QR ร้านค้าตั้งต้นสำหรับผู้จัดรายเดียว — วาง payload static (เช่น export จาก K SHOP, ขึ้นต้น `00020101…`) แล้วรายการแข่งใหม่จะใช้เป็นผู้รับเงินอัตโนมัติ; เว้นว่าง = ใช้แบบเบอร์/บัตร ปชช. |
...
│  ├─ promptpay.ts            # สร้าง PromptPay/Thai-QR payload (proxy + merchant QR แบบฝังยอด)
