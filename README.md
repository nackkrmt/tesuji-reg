...
- 📺 **ผลแข่งสด** — ฮับ `/results` รวมงานที่มีกระดานผลสด (งานที่แข่งวันนี้ขึ้นก่อน) → เข้าไปดูกระดานของงานนั้นที่ `/live/<id>` (`/live` เดิม = กระดานรวม ยังใช้ได้สำหรับกระดานที่ยังไม่ผูกกับงานไหน) — สาธารณะ ไม่ต้องล็อกอิน; รายละเอียดที่ [ระบบผลสด & กรรมการ](#ระบบผลสด--กรรมการ-live--judge)
...
- 📺 **ควบคุมการแข่งสด (เมนู `/admin/live`)** — ดูตารางการแข่ง/ผลที่บันทึกแล้ว, ส่งประกาศขึ้น banner, ลบผลจับคู่รายรอบ, และคัดลอก `tesuji.url` / `tesuji.token` สำหรับโปรแกรมจับคู่ MacMahon (การล้างข้อมูลสดทั้งหมดอยู่ที่ `/admin/reset`) — ดู [ระบบผลสด & กรรมการ](#ระบบผลสด--กรรมการ-live--judge)
...
| `/live` | กระดานรวม (ยุคงานเดียว) — ยังใช้ได้กับกระดานที่ยังไม่ผูกกับงานไหน |
| `/live/snapshot` | JSON snapshot ของสถานะการแข่งสด (`?t=<tid>` = เฉพาะงานนั้น) สำหรับ polling |
...
- 🎛️ **`/admin/live`** — หน้าควบคุมฝั่งแอดมิน: ดูตารางการแข่ง + ผลที่บันทึกแล้ว, ส่งประกาศ, ลบผลจับคู่รายรอบ (`live_delete_round`), Wall List, และคัดลอก `tesuji.url` / `tesuji.token` ไปใส่ `launcher.properties` ของโปรแกรม MacMahon — การล้างข้อมูลสดทั้งหมดทำที่ `/admin/reset`
