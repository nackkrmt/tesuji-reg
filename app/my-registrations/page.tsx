...
import { ResubmitSheet } from "@/components/registrations/ResubmitSheet";
...
function RegCard({
  ...
  // A rejection is usually a fixable slip (blurry, or the wrong amount), and
  // the registrant is the one who can fix it — but the seats went back to the
  // pool, so this is only offered while registration is still open. The RPC
  // re-checks everything and reports if the รุ่น filled up meanwhile.
  const canResubmit =
  ...
  return (
    <Card className="p-4">
      ...
      {batch.status === "pending_payment" && (
        <Link
          href={`/t/${batch.tournamentId}/register/payment?batch=${batch.id}`}
      ...
      {/* Rejected but still inside the registration window → let them replace
          the slip instead of having to ask the organiser to reopen the batch. */}
      {canResubmit && (
        <>
          <button
            type="button"
