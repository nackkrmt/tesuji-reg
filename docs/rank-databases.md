...
ผลการค้น:
- **พบรายเดียวแบบ exact/normalized (matched)** → ใช้ระดับจากฐานข้อมูลทันที
  (`rankStatus = verified`) พร้อมแสดงชื่อคนที่จับคู่ใน badge
- **พบแบบ fuzzy หรือหลายราย (multiple)** → ให้ผู้สมัครกดยืนยันเอง (พร้อมหลักฐาน
  เช่น ปีที่สอบผ่าน/เรตติ้ง) — fuzzy รายเดียวก็**ไม่** auto-apply
