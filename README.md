...
Tesuji ช่วยให้ผู้จัดการแข่งขันหมากล้อมเปิดรับสมัครออนไลน์ได้ครบวงจร — และเปิดได้ **หลายรายการแข่งพร้อมกันในระบบเดียว** (หน้าแรกคือรายชื่องาน, แต่ละงานอยู่ใต้ `/t/<id>`) ผู้เล่นสมัครเอง / โค้ช-ผู้ปกครองสมัครให้ลูกทีม, เลือกรุ่นตามระดับฝีมือ (พร้อมระบบกันโกงรุ่น), จ่ายเงินผ่าน QR ร้านค้า (K SHOP / Thai-QR) แล้วอัปสลิป (ตรวจอัตโนมัติด้วย SlipOK), และแอดมินตรวจ-ยืนยันใบสมัครได้จากหลังบ้าน หลังสมัครแล้วผู้สมัครยัง **ถอนตัว / เปลี่ยนคน / ขอเปลี่ยนรุ่น** ได้ด้วยตัวเอง, ตามผลแข่งสดได้ที่ `/results` → `/live/<id>` พร้อมคอนโซลกรรมการ (เชื่อมกับโปรแกรมจับคู่ MacMahon ได้ทั้งขาไป — export รายชื่อ — และขากลับ — นำผลเข้าฐานรางวัลที่ `/admin/awards`), และสลับภาษา **ไทย/อังกฤษ** ได้ทั้งแอป

> ชื่อที่แสดงในแอปคือ **Tesuji** (เปลี่ยนจาก "TesujiReg" เฉพาะชื่อที่แสดงผล) — ชื่อ repo / แพ็กเกจ / โปรเจกต์ Supabase ยังเป็น `tesuji-reg` ตามเดิม
...
- 🏠 **หน้าแรก = รายชื่อรายการแข่ง** — ทุกงานที่เผยแพร่แล้วอยู่ในหน้าเดียว แยกกลุ่ม **เปิดรับสมัคร / กำลังจะถึง / จบแล้ว** (ฉบับร่างไม่ขึ้น) พร้อม **ค้นหา, ชิปกรองตามสถานะ และมุมมองปฏิทินรายเดือน** (จำเดือนที่เปิดค้างไว้ให้)
- 🎯 **หน้ารายการแข่ง `/t/<id>`** — ข้อมูลงาน, รุ่น/ค่าสมัคร, ที่นั่งคงเหลือแบบเรียลไทม์, กำหนดการ (`/t/<id>/schedule`), กติกา (`/t/<id>/rules`), รายชื่อผู้เข้าแข่งขัน (`/t/<id>/participants`) และปุ่มสมัคร (`/t/<id>/register`) — ลิงก์เดิมยุคงานเดียว (`/schedule`, `/rules`, `/participants`, `/register`) ยังใช้ได้ ระบบ **redirect ไปงานที่เกี่ยวข้องที่สุด** ให้เอง (งานที่เปิดรับและใกล้ปิดที่สุด → งานถัดไปข้างหน้า → งานล่าสุดที่ผ่านมา)
...
- 👨‍👩‍👧 **ผู้เล่นในความดูแล (managed players)** — โค้ช/ผู้ปกครองบันทึกรายชื่อลูกทีมไว้ใช้สมัครซ้ำได้ (ชื่ออังกฤษไม่บังคับ เติมภายหลังได้); ช่อง **เบอร์โทร/จังหวัด/สถาบัน** มีตัวเลือก **"เหมือนเจ้าของบัญชี"** เติมให้อัตโนมัติ; ผู้เล่นที่มีใบสมัครที่ยัง active อยู่จะ **ลบไม่ได้** (กันลบทิ้งทั้งที่สมัครแข่งค้างอยู่); ลิสต์ผู้เล่นมี **ค้นหาชื่อ/เบอร์โทร + filter (ทั้งหมด/ยังไม่สมัคร/สมัครแล้ว — เทียบกับรายการแข่งปัจจุบัน) + เรียงชื่อ ก-ฮ หรือฝีมือ** ใช้ร่วมกันทั้งหน้า "ผู้เล่นในความดูแล" และขั้นตอนเลือกผู้เข้าแข่งขันตอนสมัคร (`components/players/PlayerFilterBar.tsx`)
...
- 💳 **QR ร้านค้า (K SHOP / Thai-QR) + อัปสลิป** — สร้าง QR จาก merchant QR ของร้านโดยตรง โดย **ฝังยอดเงิน (Tag 54) แต่คงค่า POI เดิมไว้** (เงินยังเข้าร้านใน K SHOP เหมือนเดิม) — บางแอปธนาคาร (เช่น **K PLUS**) ปฏิเสธ QR ที่ถูกแปลงเป็น "dynamic" จึงมีปุ่ม **"สแกนแล้วโอนไม่ได้? ลอง QR สำรอง"** สลับไปโชว์ QR ต้นฉบับของร้านแบบไม่แตะเลย (สแกนได้ทุกแอป) พร้อมปุ่มคัดลอกยอดให้กรอกเอง; อัปโหลดสลิปแล้วระบบ **ตรวจอัตโนมัติทันทีที่ส่ง** (SlipOK)
- 🟢 **ทนทานบนเบราว์เซอร์ในแอป LINE** — ตรวจจับ LINE in-app browser แล้วขึ้นแบนเนอร์เตือนพร้อมปุ่ม **"เปิดในเบราว์เซอร์ภายนอก"** (`openExternalBrowser=1`); เก็บ draft การสมัครใน `localStorage` และพก `?batch=<id>` ไว้ใน URL ตลอด flow — เผื่อ webview ถูกฆ่าตอนเปิดกล้อง/แกลเลอรีเลือกสลิป จะฟื้นกลับหน้าจ่ายเงินเดิมได้แทนที่จะเด้งกลับหน้าแรก
...
- 🔀 **ขอเปลี่ยนรุ่น (Division change)** — ย้ายที่นั่งของคนเดิมไปอีกรุ่นในงานเดียวกัน แม้ **ค่าสมัครไม่เท่ากัน**: เซิร์ฟเวอร์ preview ส่วนต่างจริง (คิดส่วนลดของใบนั้นด้วย) → **ราคาเท่ากันย้ายทันที**, **จ่ายเพิ่ม** = จ่ายส่วนต่างผ่าน QR + อัปสลิป, **ถูกลง** = กรอกบัญชีรับเงินคืน — สองทางหลังเข้าคิวรออนุมัติที่ `/admin/division-changes` และที่นั่งจะย้ายก็ต่อเมื่อแอดมินอนุมัติ
...
- ✅ **ยืนยันระดับฝีมือจาก 3 ฐานข้อมูล** — DAN / KYU / AWARD จับคู่ด้วยชื่อ (ค้น Dan ก่อน แล้ว Kyu/Award); **auto เฉพาะชื่อตรงเป๊ะทุกตัวอักษร (exact)** เท่านั้น — ชื่อที่ตรงหลัง normalize หรือ fuzzy (คล้าย) **ต้องกดยืนยันเอง** กันคนละคนชื่อพับตรงกันได้ระดับผิด; **ไม่พบ → กำหนดเป็น 15 คิวอัตโนมัติ**; จับคู่ผิด ผู้สมัคร **แก้ไขระดับฝีมือเองได้** ("ไม่ใช่ระดับนี้" → เลือกจาก dropdown เอง, เข้าคิวรอแอดมินตรวจ)
...
- 📺 **ผลแข่งสด** — ฮับ `/results` รวมงานที่มีกระดานผลสด (งานที่แข่งวันนี้ขึ้นก่อน) → เข้าไปดูกระดานของงานนั้นที่ `/live/<id>` (`/live` เปล่า ๆ เปลี่ยนเส้นทางมาที่ฮับนี้ — ทุกกระดานเป็นของงานใดงานหนึ่งเสมอ) — สาธารณะ ไม่ต้องล็อกอิน; รายละเอียดที่ [ระบบผลสด & กรรมการ](#ระบบผลสด--กรรมการ-live--judge)
- 🧭 **แถบนำทางลอย (GlassDock) 4 ปุ่ม** — หน้าแรก / ใบสมัครของฉัน / ผลการแข่งขัน / บัญชี — เมื่ออยู่ในงานใดงานหนึ่งจะสลับเป็นเมนูของงานนั้น (ภาพรวม, กำหนดการ, รายชื่อ, บัญชี + ปุ่มสมัคร)
...
- 🗂️ **หลายรายการแข่งในระบบเดียว (เมนู `/admin/tournaments`)** — สร้าง/แก้ไข/ลบรายการแข่ง และสลับสถานะ **ฉบับร่าง → เผยแพร่ → ปิดรับสมัคร** (ฉบับร่างไม่โผล่ฝั่งสาธารณะ); ทุกหน้าหลังบ้านทำงานกับ "รายการแข่งที่เลือกอยู่" ร่วมกัน (`AdminTournamentContext`) — สลับงานได้จากหัวหน้าจอโดยไม่หลุดหน้าที่เปิดค้างไว้
...
- 🔀 **คำขอเปลี่ยนรุ่น (เมนู `/admin/division-changes`)** — คิวคำขอย้ายรุ่นข้ามราคาของผู้สมัคร พร้อมสลิปส่วนต่าง (ขาจ่ายเพิ่ม) หรือบัญชีรับเงินคืน (ขาถูกลง); **กดอนุมัติแล้วเซิร์ฟเวอร์ตรวจสิทธิ์ใหม่ครบชุดกับผู้ถือที่นั่ง ณ ตอนนั้น** แล้วย้ายที่นั่ง + ปรับยอดใบสมัครให้ — อนุมัติขาถูกลงต้องแนบสลิปคืนส่วนต่างก่อน
...
- 🏅 **บันทึกรางวัลจากไฟล์ MacMahon (เมนู `/admin/awards`)** — ปิดวงจรหลังจบงาน: อัปโหลดไฟล์ `.xml` ที่เซฟจากโปรแกรม MacMahon (ไฟล์ละรุ่น) → ระบบอ่านผลทุกรอบแล้วจัดอันดับให้ (**แต้ม → SOS → SOSOS** ตามเกณฑ์ในไฟล์เอง) → เลือกจำนวนอันดับที่ให้รางวัล (สูงสุด 10 อันดับ) → **แอดมินตรวจ/แก้ได้ทุกช่องก่อนบันทึก** (เตือนเมื่อคะแนนเสมอกันพอดีตรงเส้นรางวัล, ตัดผู้เล่นตัวแทนบายทิ้ง, จับคู่ชื่อกับที่นั่งในงานเพื่อเติมคำนำหน้า/ชื่อ-สกุลให้อัตโนมัติ) → เขียนเข้าฐาน AWARD ผ่าน `admin_append_award_rows` แบบ **ต่อท้าย + idempotent** (นำเข้ารุ่นเดิมของงานเดิมซ้ำ = แทนที่ของเก่า ไม่เพิ่มซ้ำ) แล้วซิงก์ระดับฝีมือของผู้ที่ลิงก์ไว้ต่อทันที; พร้อมดาวน์โหลด **.xlsx โครงคอลัมน์เดียวกับ master sheet** ไปวางต่อในชีตกลาง — ⚠️ **master sheet ยังเป็นแหล่งข้อมูลหลัก**: การ Sync ฐาน AWARD ทั้งชุดเป็นแบบแทนที่ทั้งหมด แถวที่บันทึกที่นี่แต่ยังไม่ได้เติมลงชีตจะหายไป
- 👤 **ประวัตินักกีฬา (เมนู `/admin/people`)** — ค้นชื่อจากทะเบียนบุคคล `go_person` แล้วดูประวัติระดับฝีมือ (จากฐาน DAN/KYU/AWARD) + ประวัติการสมัครแข่งของคนนั้นทั้งระบบในหน้าเดียว
...
- 📺 **ควบคุมการแข่งสด (เมนู `/admin/live`)** — ดูตารางการแข่ง/ผลที่บันทึกแล้ว, ส่งประกาศขึ้น banner, ลบผลจับคู่รายรอบ, และคัดลอก `tesuji.url` / `tesuji.token` **ของงานที่เลือกอยู่** สำหรับโปรแกรมจับคู่ MacMahon (token แยกต่องาน สร้างใหม่ได้ถ้าหลุด; การล้างข้อมูลสดของงานอยู่ที่ `/admin/reset`) — ดู [ระบบผลสด & กรรมการ](#ระบบผลสด--กรรมการ-live--judge)
- 🧑‍⚖️ **จัดการกรรมการ (เมนู `/admin/judges`)** — แต่งตั้ง/ถอดถอนกรรมการ **ของงานที่เลือกอยู่** เป็นรายบัญชี (ตาราง `tournament_judge` — กรรมการแต่ละงานแยกกัน) ผ่าน `admin_set_judge` / `admin_list_judges` — กำหนดรุ่นเริ่มต้นต่อคน, ถอดสิทธิ์ต้องยืนยันก่อน
...
|---|---|
| **สาธารณะ / ผู้ใช้** | |
| `/` | **รายชื่อรายการแข่งทั้งหมด** — ค้นหา + ชิปกรองสถานะ + ปฏิทินรายเดือน |
| `/t/[tid]` | หน้ารายการแข่งหนึ่งงาน (ข้อมูลงาน, รุ่น/ค่าสมัคร, ที่นั่งคงเหลือ) |
| `/t/[tid]/schedule`, `/t/[tid]/rules`, `/t/[tid]/participants` | กำหนดการ / กติกา / รายชื่อผู้เข้าแข่งขันของงานนั้น |
| `/t/[tid]/register` → `/applicant` → `/categories` → `/payment` → `/success` | ขั้นตอนสมัครแข่งของงานนั้น (+ `/expired` เมื่อหมดเวลาจอง; `/payment?batch=<id>` = กลับไปจ่ายใบที่ค้างจาก "ใบสมัครของฉัน") |
| `/schedule`, `/rules`, `/participants`, `/register` | ลิงก์เดิมยุคงานเดียว — redirect ไปงานที่เกี่ยวข้องที่สุดโดยอัตโนมัติ |
| `/login`, `/signup` | เข้าสู่ระบบ / สมัครสมาชิก |
| `/forgot-password`, `/reset-password` | ขอลิงก์รีเซ็ตรหัสผ่าน / ตั้งรหัสผ่านใหม่ |
| `/account`, `/profile` | ฮับบัญชี (โปรไฟล์ + ผู้เล่นในความดูแล) / โปรไฟล์ตัวเอง |
| `/my-registrations` | ใบสมัครของฉัน (ทุกงาน) — สถานะ, ปุ่มกลับไปจ่ายเงิน, **ถอนตัว / เปลี่ยนคน / ขอเปลี่ยนรุ่น** |
| `/results` | ฮับผลการแข่งขัน — งานที่มีกระดานผลสด (งานวันนี้ขึ้นก่อน) |
| **ผลสด & API** | |
| `/live/[tid]` | กระดานผลแข่งสดของงานนั้น (raw HTML — ไม่ผ่าน React app) |
...
│  ├─ page.tsx                # หน้าแรก = รายชื่อรายการแข่ง
│  ├─ t/[tid]/                # หน้าของแต่ละรายการแข่ง (schedule, rules, participants, register/*)
│  ├─ schedule/ rules/ participants/ register/   # ลิงก์เดิมยุคงานเดียว → redirect ไปงานที่เกี่ยวข้อง
│  ├─ login/ signup/ forgot-password/ reset-password/   # auth
│  ├─ account/ profile/       # ฮับบัญชี (+ ผู้เล่นในความดูแล) / โปรไฟล์ตัวเอง
│  ├─ my-registrations/       # ใบสมัครของฉัน (+ ถอนตัว/เปลี่ยนคน/ขอเปลี่ยนรุ่น)
│  ├─ results/                # ฮับผลการแข่งขัน (ลิงก์ไปกระดานของแต่ละงาน)
│  ├─ live/                   # กระดานผลสด raw HTML — [tid]/ = กระดานของงานนั้น, snapshot/ = JSON
│  ├─ judge/[key]/            # คอนโซลกรรมการ (raw HTML)
│  ├─ api/divisions/          # REST API สำหรับโปรแกรมจับคู่ MacMahon
│  ├─ admin/                  # หลังบ้าน (tournaments, tournament, categories, rules, registrations, withdrawals, division-changes, codes, live, judges, database, awards, people, institutes, reset)
...
│  ├─ registrations/          # WithdrawSheet, SwapSeatSheet, ChangeDivisionSheet (ถอนตัว/เปลี่ยนคน/เปลี่ยนรุ่น)
│  ├─ tournament/             # TournamentDetailClient, TournamentProvider, RegisterCta, LegacyRedirect
│  ├─ players/                # PlayerFilterBar — ค้นหา/filter/sort ผู้เล่น (ใช้ร่วม account + register/applicant)
│  ├─ home/                   # TournamentListClient, TournamentCalendar, CategoryTable
│  ├─ results/                # ResultsHubClient — ฮับผลการแข่งขัน
...
│  ├─ tournament-list.ts      # จัดเฟส (เปิดรับ/กำลังจะถึง/จบแล้ว) + จัดกลุ่มหน้าแรก
│  ├─ age.ts                  # คำนวณอายุจากวันเกิด + กฎ eligibility ช่วงอายุ
│  ├─ provinces.ts            # รายชื่อ 77 จังหวัด (province picker)
│  ├─ promptpay.ts            # สร้าง Thai-QR payload จาก merchant QR (K SHOP) แบบฝังยอด (คง POI เดิม) + QR สำรอง
│  ├─ browser.ts              # ตรวจจับ LINE in-app browser + ลิงก์เปิดเบราว์เซอร์ภายนอก
...
│  ├─ macmahon-xml.ts         # parser ไฟล์ .xml ของ MacMahon + จัดอันดับ (แต้ม → SOS → SOSOS)
│  ├─ award-sheet.ts          # สร้าง .xlsx ผู้ได้รางวัล โครงคอลัมน์ตรง master sheet
│  ├─ download.ts             # ตัวช่วยดาวน์โหลดไฟล์ในเบราว์เซอร์ (ใช้ร่วมหน้า export/awards)
...
│  ├─ admin-reset.ts          # นิยามกลุ่มข้อมูลของหน้ารีเซ็ต + dependency ระหว่างกลุ่ม
...
- 🎛️ **`/admin/live`** — หน้าควบคุมฝั่งแอดมิน: ดูตารางการแข่ง + ผลที่บันทึกแล้ว, ส่งประกาศ, ลบผลจับคู่รายรอบ (`live_delete_round`), Wall List, และคัดลอก `tesuji.url` / `tesuji.token` **ของงานที่เลือก** (+ ลิงก์กรรมการ, สร้าง token ใหม่ได้) ไปใส่ `launcher.properties` ของโปรแกรม MacMahon — ทุกอย่างบนหน้านี้เป็นของงานที่เลือกใน picker; การล้างข้อมูลสดของงานทำที่ `/admin/reset` (เฉพาะงานที่เลือก)
...
- **RPC สำคัญ (จัดกลุ่ม):**
  ...
  | เปลี่ยนรุ่น | `preview_division_change` / `request_division_change` | คิดส่วนต่างค่าสมัครฝั่งเซิร์ฟเวอร์ (คิดส่วนลดของใบนั้นด้วย) → ราคาเท่ากันย้ายทันที, ต่างราคาสร้างคำขอรออนุมัติ (แนบสลิปส่วนต่าง หรือบัญชีรับเงินคืน); คำขอของตัวเองอ่านตรงจาก `seat_division_change` ผ่าน RLS เฉพาะเจ้าของ |
  | | `admin_list_division_changes` / `admin_resolve_division_change` | แอดมินดู/อนุมัติ-ปฏิเสธคำขอ — อนุมัติจะตรวจสิทธิ์ใหม่ครบชุดกับผู้ถือที่นั่งปัจจุบัน, ย้ายที่นั่ง แล้วปรับยอดใบสมัคร (ขาถูกลงต้องแนบสลิปคืนส่วนต่าง) |
  ...
  | | `admin_append_award_rows` | **ต่อท้าย**แถวรางวัลจากหน้า `/admin/awards` (นำเข้าไฟล์ MacMahon XML) — ต่างจาก `admin_import_rank_database` ที่แทนที่ทั้งแหล่ง; ลบแถวเดิมที่ `(event_name, event_date, rank_in_category, category)` ตรงกันก่อนเขียนใหม่ (re-import ไม่ซ้ำ) แล้วรีเฟรชทะเบียน + ซิงก์ระดับต่อในทรานแซกชันเดียว |
  | | `admin_search_person_history` / `person_rank_history` | ค้นบุคคลในทะเบียน + ประวัติระดับฝีมือ/การสมัครแข่ง (หน้า `/admin/people`) |
  ...
  | จัดการงาน/รีเซ็ต | `upsert_tournament` / `set_tournament_status` / `admin_delete_tournament` / `update_tournament_rules` | สร้าง/แก้รายการแข่ง, สลับสถานะ `draft`/`published`/`closed` (ฉบับร่างถูกซ่อนจากฝั่งสาธารณะฝั่งเซิร์ฟเวอร์), ลบงาน, บันทึกเฉพาะเนื้อหากฎ กติกา |
  | | `admin_selective_reset` | รีเซ็ตหลังจบงานแบบติ๊กเลือกกลุ่ม (`p_targets[]`) — ตรวจ dependency + คำยืนยันซ้ำฝั่งเซิร์ฟเวอร์; เก็บบัญชีผู้เรียกไว้เสมอ (`p_keep_uid`) เรียกผ่าน Edge Function `admin-reset` เท่านั้น (service role) |
  | Roles | `is_admin_me` | เช็คว่า session ปัจจุบันมี role `admin` (ใช้ gate UI ฝั่งหน้าบ้าน) |
  | | `admin_set_judge` / `admin_list_judges` / `judge_my_assignments` | แต่งตั้ง/ลิสต์กรรมการ **ต่องาน** (`tournament_judge`) + งานที่ผู้ใช้เป็นกรรมการพร้อมลิงก์คอนโซล |
  | Live/Judge | ตระกูล `live_*` (`live_upsert_division`, `live_replace_round`, `live_submit_result`, `live_set_standings`, `live_set_force`, `live_toggle_checkin`, `live_set_config`, `live_clear_tournament`, …) | เขียนข้อมูลแข่งสด — ทุกตัว gate ด้วย `_can_write_division` (แอดมิน หรือ token ของงานที่รุ่นนั้นสังกัด); `live_upsert_division` หางานจาก token แล้วคืน id ภายในของรุ่น |
  | | `live_get_token` / `live_rotate_token` / `judge_get_token` / `live_check_token` / `live_token_tournament` | อ่าน/สร้างใหม่ token ของงาน (แอดมิน/กรรมการของงานนั้น) + แปลง token เป็นงานสำหรับคอนโซลกรรมการและ API |
