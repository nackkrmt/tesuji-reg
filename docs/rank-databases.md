...
3. **fuzzy** — ความคล้าย (`pg_trgm` similarity **> 0.7** บนชื่อเต็มต่อกัน — เข้มเพื่อ
   ลดชื่อคนละคนที่บังเอิญคล้ายกันหลุดมา)
...
ผลการค้น:
- **เจอ exact ตรงเป๊ะตัวเดียว (matched)** → ใช้ระดับจากฐานข้อมูลทันที
  (`rankStatus = verified`) พร้อมแสดงชื่อคนที่จับคู่ใน badge — **เฉพาะชื่อตรงเป๊ะ
  ทุกตัวอักษรเท่านั้น**ที่ auto (ชื่อที่ตรงเป๊ะไม่กำกวม)
...
2. **auto-link** คนที่ยังไม่มีลิงก์ ซึ่งชื่อ normalize ตรงกับแถวในทะเบียน (เติมเฉพาะที่ยัง
   ว่าง — ไม่ทับลิงก์ที่ผู้ใช้เลือกไว้เอง) — รวมถึง **ทับระดับที่ผู้ใช้เคยกรอกเอง (pending)**
