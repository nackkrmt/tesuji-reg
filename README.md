...
- 📄 **"ใบสมัครของฉัน"** — ดูใบสมัครทั้งหมดของตัวเองพร้อมสถานะ; ใบที่ **รอชำระเงิน** มีปุ่ม **"ชำระเงิน / ดู QR"** กลับไปจ่ายต่อได้เสมอ แม้ปิดแท็บ/แอปไปแล้วก่อนอัปสลิป (สร้าง QR ใหม่จากข้อมูลฝั่งเซิร์ฟเวอร์)
...
- 📖 **อ่านกติกาในแอปได้เลย** — หน้า `/rules` ฝังตัวอ่าน PDF ในตัว (เลื่อนหน้า + ซูมได้ ลื่นบนมือถือ) หรือกดเปิดเต็มจอ/ดาวน์โหลด
...
- 📄 **กติกาเป็นไฟล์ PDF** — อัปโหลด PDF แล้วผู้เล่น **อ่านในแอปได้เลย** (ตัวอ่านในตัว เลื่อน/ซูม) หรือเปิดเต็มจอ/ดาวน์โหลด
...
- 📋 ตรวจ-ยืนยัน/ปฏิเสธใบสมัคร + ดูสลิป — **ค้นหารายชื่อ** (แสดงเป็นรายคน + ยอดที่ต้องโอน + จังหวัด/สถาบัน/สถานะ PDPA), หน้าใบสมัครโชว์ **ชื่อเจ้าของบัญชีที่สมัคร + อีเมล**, **แก้ไข/ลบผู้สมัครรายคนหรือทั้งใบ** (ปรับที่นั่ง/ยอดเงินให้อัตโนมัติ), หน้ารวมรายชื่อผู้เข้าแข่งขัน
...
- 🧨 **รีเซ็ตหลังจบงาน (เมนูแยก `/admin/reset`)** — 3 ปุ่มแยก: **ล้างใบสมัครทั้งหมด** / **ลบรุ่นทั้งหมด** / **ลบทัวร์นาเมนต์ทั้งหมด**; กันพลาดด้วยการ **พิมพ์ชื่อรายการยืนยัน** (เซิร์ฟเวอร์ตรวจซ้ำอีกชั้น) — ลบถาวรย้อนกลับไม่ได้ (สถาบัน/คำค้นเป็นข้อมูลกลาง ไม่ถูกแตะ)
...
| `/my-registrations` | ใบสมัครของฉัน — สถานะ + ปุ่มกลับไปจ่ายเงินสำหรับใบที่รอชำระ |
| `/participants` | รายชื่อผู้เข้าแข่งขัน (สาธารณะ) — แสดงสถานะ ยืนยันแล้ว / รอตรวจสลิป |
| `/admin/*` | หลังบ้าน: `login`, `tournament`, `categories`, `registrations`, `registrations/[id]`, `database`, `institutes`, `codes`, `reset` |
...
| [react-pdf](https://github.com/wojtekmaj/react-pdf) | ^7.7 | ตัวอ่าน PDF กติกาในแอป (pin v7 — pdf.js v3; v5/v10 พังกับ Next 14 webpack) — ตั้ง `isEvalSupported:false` กัน CVE-2024-4367 |
...
│  └─ migrations/              # SQL migrations (ตาราง/RPC โค้ดส่วนลด + ชุด hardening `20260701_*` + เพดานรางวัล 1 คิว `20260705_0003`)
...
- **ตารางหลัก:** `tournament` (กำหนดการเก็บเป็น JSON ใน `schedule_text`, กติกาเก็บ URL ของ PDF ใน `rules_text`), `category` (ตัวนับ `seats_taken` + เช็คห้ามจองเกิน + `combinable_category_ids`), `registration_batch` (มี `slip_verify_status` / `slip_verify_data` / `slip_verified_at` + `promo_code` / `discount_thb`), `registration_seat`, `seat_hold`, `seat_hold_line`, `profile`, `managed_player`, `app_config`, `go_player_database`, `award_limit_exemption` (รายชื่อยกเว้นเพดานรางวัล 1 คิว), `go_institute` (มี `keywords[]` = คำค้น/alias), `institute_merge` (ประวัติการรวมสถาบันแบบ reversible), `promo_code` / `promo_redemption` (โค้ดส่วนลด/สมัครฟรี ต่อรายการแข่ง + ประวัติการใช้)
...
- **RPC สำคัญ:** `reserve_seats` (จองที่นั่ง all-or-nothing + กันโกงรุ่นทั้งระดับฝีมือ/อายุ + กันสมัครซ้ำ + เช็ครุ่นที่เล่นควบกันได้ + เพดานรางวัล 1 คิว + snapshot จังหวัด/สถาบัน/PDPA), `release_expired_holds` (คืนที่นั่งหมดเวลา — pg_cron ทุกนาที + lazy), `search_go_player_database`, `replace_go_player_database_source`, `award_limit_status` (เช็กเพดานรางวัล 1 คิว — ใช้ทั้งด่านใน `reserve_seats` และคำเตือนฝั่ง client) + `admin_add_award_exemption` / `admin_remove_award_exemption` / `admin_list_award_exemptions` (จัดการรายชื่อยกเว้น), `find_or_create_institute` (พิมพ์เพิ่มสถาบันใหม่), `upsert_institute` / `delete_institute` / `purge_institute` / `admin_list_institutes` / `admin_institute_counts` (จัดการสถาบัน + คำค้น + นับผู้สมัครต่อสถาบัน), `merge_institute` / `unmerge_institute` / `list_institute_merges` (รวม/แยกคืนสถาบัน), `admin_update_seat` / `admin_delete_seat` / `admin_delete_batch` (แก้ไข/ลบผู้สมัคร), `admin_clear_registrations` / `admin_clear_categories` / `admin_delete_tournament` (รีเซ็ตหลังจบงาน — ต้องส่งชื่อรายการยืนยัน), `apply_promo` (ผู้สมัครใส่โค้ด → คิดส่วนลดลง `total_amount_thb`; นับการใช้ตอน `submit_registration` แบบ atomic, ฟรี → ยืนยันอัตโนมัติ) + `admin_upsert_promo` / `admin_delete_promo` / `admin_list_promos` และ RPC ฝั่งแอดมินสำหรับจัดการรายการแข่ง/รุ่น/ใบสมัคร
- **Edge Functions:** `verify-slip` (ตรวจสลิปผ่าน SlipOK ฝั่ง server — มีโหมดเดโม + action `view` คืน **signed URL** อายุสั้นให้แอดมินดูสลิป; อ่านสลิปจาก storage ของตัวเองเท่านั้น = กัน SSRF), `sync-go-database` (ดึง Google Sheet CSV เลี่ยง CORS — จำกัดปลายทางเฉพาะโฮสต์ Google + ตามรีไดเรกต์แบบตรวจโฮสต์ทุก hop = กัน SSRF)
- **ความปลอดภัย:** RLS เปิดสิทธิ์เฉพาะเจ้าของบนตาราง PII (`profile`/`managed_player`); anon/ผู้ใช้อ่านได้แค่ `tournament`/`category`/`go_institute`; ฐานข้อมูลระดับฝีมือ `go_player_database` **เข้าถึงผ่าน RPC ค้นหาเท่านั้น** (ไม่เปิดอ่านทั้งตาราง); RPC ฝั่งผู้ใช้ที่รับ batch id (`get_batch_public` / `submit_registration` / `release_batch` / `apply_promo`) **ตรวจความเป็นเจ้าของด้วย `auth.uid()`** ทุกตัว (แอดมินอ่านใบสมัครผ่าน `admin_get_batch` แยก); ผู้สมัครกรอกฟอร์มถูก **ตรวจซ้ำฝั่งเซิร์ฟเวอร์** ใน `reserve_seats` (กันเลี่ยงฟอร์ม); ที่เหลือผ่าน RPC / Edge Function ที่ป้องกันด้วย admin passphrase (`app_config.admin_secret`); คีย์ SlipOK อยู่ใน Edge Function secrets ไม่หลุดมาเบราว์เซอร์; ตั้ง **security headers + CSP** ใน `next.config.mjs`
- **Storage:** bucket สาธารณะ `tesuji` สำหรับแบนเนอร์ + ไฟล์ PDF กติกา (จำกัดชนิด/ขนาดไฟล์); **สลิปการชำระเงินอยู่ในบัคเก็ตส่วนตัว `tesuji-slips`** ไม่เปิดสาธารณะ — แอดมินดูผ่าน signed URL อายุสั้น, `verify-slip` อ่านด้วย service role
